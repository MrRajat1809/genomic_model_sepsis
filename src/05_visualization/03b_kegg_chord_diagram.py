"""
03b_kegg_chord_diagram.py

Generates Panel B: KEGG Pathways Chord Diagram.
Maps the optimal biomarker genes to the top enriched KEGG pathways.
...
# ==========================================
def main():
    print("[*] Generating Panel B: KEGG Chord Diagram...")
    ...
    # ---------------------------------------------------------
    print(f"    -> Extracting top {MAX_PATHWAYS} {TARGET_DB} pathways...")
    df = pd.read_csv(csv_path)
    df = df[df['Gene_set'] == TARGET_DB].copy()
    
    if df.empty:
        print(f"[!] No KEGG pathways found. Ensure '{TARGET_DB}' was used.")
        return
